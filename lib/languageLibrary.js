...
const languageLibrary = {
  accordion: {
    ...
    about_project_language_feature: {
      EN: 'A unique feature of this app is the scalable language database, which can be easily expanded to include additional languages, supplying both the frontend and backend (API routes and email dispatch) with text content.',
      DE: 'Ein besonderes Feature der App ist die skalierbare Sprachendatenbank, die problemlos um zusätzliche Sprachen erweitert werden kann und sowohl das Frontend als auch das Backend (API-Routen und E-Mail-Versand) mit Textinhalten versorgt.',
    },
